        ...
        if '-f' in self.sys_argvs:
            ...
            try:
                ...
                for os_type,os_type_data in self.config_data_dic.items(): #按照不同的操作系统单独生成一份配置文件
                    for section_name,section_data in state_data.items(): # section_name == apache
                        print('Section:',section_name)
                        for mod_name,mod_data in section_data.items(): # mod_name == user,group（模板.方法），
                            base_mod_name = mod_name.split(".")[0]
                            module_obj=self.get_module_instance(base_mod_name=base_mod_name,os_type=os_type)#生成不同操作系统的类
                            module_parse_result = module_obj.syntax_parser(section_name,mod_name,mod_data,os_type)# 语法解析
