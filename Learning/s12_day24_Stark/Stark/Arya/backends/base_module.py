...
class BaseSaltModule(object):
    ...
    def get_selected_os_types(self):
        '''
        匹配系统类型,按照系统类型生成空空的列表val
        {<Host: ubuntu test>} host_list中为数据对象
    ...
    def process(self):
        ...
        :return:
        '''
        self.fetch_hosts() # 提取主机
        self.config_data_dic = self.get_selected_os_types() # 提取主机类型 ububtu and windows or ...
    def require(self,*args,**kwargs):
        ...
        self.require_list = []
        for item in args[0]: #args[0]为方法里的子参数 事例：user:root
            for mod_name,mod_val in item.items():
                ...
                print("require_condition:_________________",require_condition)
    ...
    def is_required(self,*args,**kwargs):
        '''
        yml文件有依赖，对应模块没写is_required方法则退出报错
        :param args:
        :param kwargs:
        :return:
        '''
    ...
    def get_module_instance(self,*args,**kwargs):
        ...
        if os.path.isfile(plugin_file_path):
            ...
            if hasattr(module_file, special_os_module_name): #判断有没有根据操作系统的类型进行特殊解析 的类，在这个文件里
                ...
                module_instance = getattr(module_file, base_mod_name.capitalize()) # 没有针对操作系统类型，则采取默认的
            #开始调用 此module 进行配置解析
        ...
        for state_item in mod_data:
            ...
            for key,val in state_item.items():
                if hasattr(self,key):
                    ...
                    state_func(val,section=section_name,os_type=os_type) # 执行最底层的方法  事例：uid:87
                else:
                    exit("Error:module [%s] has no method [%s]" %( mod_name,key ))
        else:
            if '.' in mod_name:
                base_mod_name,mod_action = mod_name.split('.') #mod_action 执行动作
