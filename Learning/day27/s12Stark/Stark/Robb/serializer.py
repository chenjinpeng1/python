...
class ClientHandler(object):
    ...
    def fetch_configs(self):
        try:
            ...
            template_list= list(host_obj.templates.select_related()) # 获取主机监控模板

            for host_group in host_obj.host_groups.select_related():
                template_list.extend( host_group.templates.select_related() ) # 获取组监控模板
            print(template_list) # 打印（去重问题？）
            for template in template_list:
                #print(template.services.select_related())

                for service in template.services.select_related(): #loop each service
                    print(service)
                    self.client_configs['services'][service.name] = [service.plugin_name,service.interval] # 获取服务插件名，和间隔时间
        ...
        return  self.client_configs # 返回客户端的服务器插件名，间隔时间 cpu.30
