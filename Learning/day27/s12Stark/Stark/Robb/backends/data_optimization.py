...
class DataStore(object):
    ...
    def process_and_save(self):
        '''
        processing data and save into redis # 处理处理过后存入redis
        :return:
        '''
        ...
        if self.data['status'] ==0:# service data is valid
            for key,data_series_val in settings.STATUS_DATA_OPTIMIZATION.items():
                data_series_key_in_redis = "StatusData_%s_%s_%s" %(self.client_id,self.service_name,key) #StatusData_id_LinuxCPU_60mins
                ...
                if data_series_val[0] == 0:#=0代表数据不优化，最近的数据 ，直接存
