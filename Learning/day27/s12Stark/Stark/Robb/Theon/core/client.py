...
class ClientHandle(object):
    ...
    def load_latest_configs(self):
        ...
        url = "%s/%s" %(settings.configs['urls']['get_configs'][0], settings.configs['HostID']) #请求url,主机ID
        latest_configs = self.url_request(request_type,url) # 获取配置信息
        latest_configs = json.loads(latest_configs) # 解析数据
        self.monitored_services.update(latest_configs) # 更新配置信息数据到字典
    ...
    def forever_run(self):
        ...
        while not exit_flag:
              if time.time() - config_last_update_time > settings.configs['ConfigUpdateInterval']:# 当前时间 - 上一次执行的时间 > 监控配置更新间隔时间
                  self.load_latest_configs()
                  print("Loaded latest config:", self.monitored_services)
                  config_last_update_time = time.time() # 更新下本次执行的时间
              #start to monitor services

              for service_name,val in self.monitored_services['services'].items(): # 循环刚才获取到的配置信息数据字典
                  if len(val) == 2:# means it's the first time to monitor
                      self.monitored_services['services'][service_name].append(0) # 这里的0代表第一次执行时候判断间隔的时间
                  monitor_interval = val[1]
                  last_invoke_time = val[2]
                  if time.time() - last_invoke_time > monitor_interval: #needs to run the plugin # 当前时间 - 上一次执行的时间 > 监控间隔时间
                      print(last_invoke_time,time.time())
                      self.monitored_services['services'][service_name][2]= time.time() # 时间重置
    ...
    def invoke_plugin(self,service_name,val):
        ...
        if hasattr(plugin_api,plugin_name):
            func = getattr(plugin_api,plugin_name) # 反射取各个插件返回的结果
            ...
            } # 拼接最终返回的数据，带上【hostid,服务名称，数据】

            request_action = settings.configs['urls']['service_report'][1] # 请求类型 post
            request_url = settings.configs['urls']['service_report'][0] # 请求url
