...
from api.module import saltapi
from ESZCMDB import settings
from cmdb import models
class Asset(object):
    def __init__(self,request):
        self.request = request
        self.mandatory_fields = ['sn','asset_id','asset_type'] #强制字段，必须指定这些字段
        self.field_sets = { # 暂没用
            'asset':['manufactory'],
        ...
        # 初始化调用saltapi 取出数据
        self.salt = saltapi.SALT_API(url=settings.SALT_API["url"],username=settings.SALT_API['username'],password=settings.SALT_API['password'])
        hostid = json.loads(self.request.POST.get("hostid"))
        self.saltid = models.Asset.objects.get(id=hostid)
        self.grains = self.salt.remote_execution_func(tgt=self.saltid,fun="grains.items",arg='')
    ...
    def mandatory_check(self,data,only_check_sn=False):
        for field in self.mandatory_fields:
            ...
            if not data.get(field): # 如果客户端传来的键值不在定义的列表里，写入报错信息
                self.response_msg('error','MandatoryCheckFailed', "hostid:[%s] ,The field [%s] is mandatory and not provided in your reporting data" %(field) )
        else:
            if self.response['error']:return False # 如果response里有报错信息，直接return返回
        try:

            if not only_check_sn: # only_check_sn 为False时执
                self.asset_obj = models.Asset.objects.get(id=int(data['asset_id']),sn=data['sn'])
                print("------asset_obj:",self.asset_obj)
            else:
    ...
    def data_grep(self):
        '''
        处理salt传入过来的数据，获取想要的数据
        :return:
        '''
        data = {}
        for hostid,hostdata in self.grains.items():
            print(hostdata)
            data["sn"] = hostdata["assetinfo"]["UUID"]
            data["asset_type"] = 'server'
            data["asset_id"] = hostid
        # data = []
    ...
    def data_is_valid(self):
        datas = self.data_grep() #取出数据
        if datas:
            try:
                # data = json.loads(data)
                self.mandatory_check(datas) # 惯例检查
                print("mandaroty:")
                self.clean_data = datas
