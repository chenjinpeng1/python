...
def collect():
    ...
    raw_data = {}
    for key in filter_keys: # get basic sys info
        try:
    ...
    data = {"asset_type":'server'} # 写死 只能是server
...
class DiskPlugin(object):
    ...
    def linux(self):
        ...
        try:
            ...
            shell_command = "sudo %s/MegaCli  -PDList -aALL" % script_path  # MegaCli是获取raid卡的一些信息
            output = subprocess.Popen(shell_command,stdout=subprocess.PIPE,shell=True)
            result['physical_disk_driver'] = self.parse(output.stdout.read())
        except Exception as e:
            result['error'] = e
        return result
