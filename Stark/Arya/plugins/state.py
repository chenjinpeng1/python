...
class State(BaseSaltModule):
    ...
    def apply(self):
        ...
        if '-f' in self.sys_argvs:
            yaml_file_index = self.sys_argvs.index('-f') + 1 #取出文件索引
            try:
                yaml_filename = self.sys_argvs[yaml_file_index] # 文件名称
                state_data = self.load_state_files(yaml_filename) # 文件内容
                print("condig-data-dict---------",self.config_data_dic)
                    ...
                    for section_name,section_data in state_data.items():
                        ...
                        for mod_name,mod_data in section_data.items():
                            ...
                            if os.path.isfile(plugin_file_path):
                                ...
                                #开始调用 此module 进行配置解析
                                module_obj = module_instance(self.sys_argvs,self.db_models,self.settings) # 不同系统生成的实例化
                                # print('--------------',module_obj)
                                # print("module_obj:",module_obj)
                                ret = module_obj.syntax_parser(section_name,module_instance,mod_data ) #解析结果
                                if ret: #如果为True
                                    module_obj.process_result()
                            else:
