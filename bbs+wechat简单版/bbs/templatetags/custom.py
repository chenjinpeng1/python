    ...
    for k,v in comment_tree.items():
        ...
        if k.comment_type == 1:
            ...
            <div class="user_info">
                ...
                <span id=%s name=author_user style="margin-right:20px">%s</span>
                <span style="margin-right:20px">%s</span>

            </div>
            <div class="pl-content" style='margin-top:20px'>
                <span >%s</span>
                <span style="padding-right:20px" class='glyphicon glyphicon-thumbs-up pull-right goodlike'>%s</span>
            ...
            '''%(margin_num,img,k.id,k.user,k.date.strftime('%Y-%m-%d %T'),k.comment,comment_counts['thumb_count'],comment_counts['comment_count'])
...
def build_comment_tree(comment_tree):
    ...
    for k,v in comment_tree.items():
        ...
        if k.comment_type == 1:
            ...
            <hr/>
            <div class=A-comment>
             <div class="pl-box-wrap" style="margin-left:0px">
             <div class="user_info">
                <span style="margin-right:20px"><img class="small_img" src="/static/%s"></span>
                <span id=%s name="author_user" style="margin-right:20px">%s</span>
                <span style="margin-right:20px">%s</span>
             ...
             '''%(img,k.id,k.user,k.date.strftime('%Y-%m-%d %T'),k.comment,comment_counts['thumb_count'],comment_counts['comment_count'])
            comment_html+=first_row
            if v:
