...
class FibonacciRpcClient(object):
    def __init__(self): # 初始化配置
        self.connection = pika.BlockingConnection(pika.ConnectionParameters(host='localhost')) # 创建链接
        self.channel = self.connection.channel() # 建立隧道
        self.channel.exchange_declare(exchange="cmd",type="fanout") # 建立交换器
        result = self.channel.queue_declare(exclusive=True) #生产随机Q ，exclusive=True链接断开后自动删除这个Q
